...
class DQN:
	...
	def __init__(self, env):
		...
		checkpoint = tf.train.get_checkpoint_state("exp03saved_networks")
		...
		summary_writer = tf.summary.FileWriter('results/Exp03Graph', graph=self.session.graph)
	...
	def train_Q_network(self, reward, done):
		...
		# 	self.saver.save(self.session, 'results/exp03saved_networks/' + 'network' + '-dqn', global_step=self.time_step)
...
def main():
	...
	# restore lists
	write_file('results\\exp03reve\\episodes.txt', agent.episode_array)
	write_file('results\\exp03reve\\time_line_q.txt', agent.time_line)
	write_file('results\\exp03reve\\reward_array.txt', agent.reward_array)
	write_file('results\\exp03reve\\max_q_array.txt', agent.max_q_array)
	print("finished!")
