...
# from image_processing import *  # functions to process images


sys.path.append("game/")
GAME = 'CartPole-v0'  # the name of the game being played
...
def trainNetwork(s, readout, W_fc1, W_fc2, sess):
    ...
    writer = tf.summary.FileWriter(r"Exp01Graph/", sess.graph)
    ...
    with tf.name_scope('reward_per_100_steps'):
        ...
        reward_sum_step = tf.summary.scalar('reward_per_100 steps', reward_step)
    ...
    with tf.name_scope('500per_average_qMax'):
        q_max = tf.Variable(0.0, name='q_max_average')
        q_max_sum = tf.summary.scalar('500per_average_qMax', q_max)
    ...
    checkpoint = tf.train.get_checkpoint_state(r"Exp01_saved_networks")
    ...
    while True:
        ...
        x_t1_colored, r_t, terminal, _ = game.step(action_index)
        r_t = -1 if terminal else 0.01  # important!!! change the reward
        # x_t1 = cv2.cvtColor(cv2.resize(x_t1_colored, (80, 80)), cv2.COLOR_BGR2GRAY)
        x_t1 = cv2.resize(x_t1_colored, (80, 80))
        ...
        if t > OBSERVE:
            ...
            if t % 100 == 0:
                ...
                writer.add_summary(re_step, t * 100)
        ...
        if t % 10000 == 0:
            saver.save(sess, 'Exp01_saved_networks/' + GAME + '-dqn', global_step=t)
