...
def trainNetwork(s, readout, W_fc1, W_fc2, sess):
	...
	while t < 100000:
		...
		x_t1_colored, r_t, terminal, ball_x1, bat_mid1 = game_state.frame_step(a_t)				# returns new game images and new reward.
		...
		if ball_x1 > bat_mid1:
			...
			rule_action = [1, 0, 0]
		if rule_action == a_t.tolist():
				action_count += 1
		elif pic_num < 10 and t % 60 == 0:
			cv2.imwrite("reve" + str(t) + ".png", x_t1_colored)
			pic_num += 1
		...
		print("TIMESTEP", t, "/ STATE", state,
			...
			"/ Q_MAX %e" % np.max(readout_t), "/ action_count", action_count)

	time_line_file = open(r'reve_time_line.txt', 'w')
	for word in time_line:
		time_line_file.write(str(word))
		time_line_file.write('\n')
	time_line_file.close()

	action_times_array_file = open(r'reve_action_times_array.txt', 'w')
