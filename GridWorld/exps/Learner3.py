...
actions = World3.actions
states = []
Q = {}
E = {}
for i in range(World3.x):
    for j in range(World3.y):
        states.append((i, j))
...
for state in states:
    ...
    for action in actions:
        ...
        temp_e[action] = 0.0
        World3.set_cell_score(state, action, temp[action])
    Q[state] = temp
    E[state] = temp_e

for (i, j, c, w) in World3.specials:
    for action in actions:
        Q[(i, j)][action] = w
        World3.set_cell_score((i, j), action, w)
...
def inc_Q(s, a, alpha, inc):
    ...
    World3.set_cell_score(s, a, Q[s][a])
...
def run():
    ...
    for episode_num in range(40):
        steps = 0
        score = 0
        while not World3.has_restarted():
            # Do the action
