...
actions = World2.actions
states = []
Q = {}
E = {}
for i in range(World2.x):
    for j in range(World2.y):
        states.append((i, j))
...
for state in states:
    ...
    for action in actions:
        ...
        temp_e[action] = 0.0
        World2.set_cell_score(state, action, temp[action])
    Q[state] = temp
    E[state] = temp_e

for (i, j, c, w) in World2.specials:
    for action in actions:
        Q[(i, j)][action] = w
        World2.set_cell_score((i, j), action, w)
...
def inc_Q(s, a, alpha, inc):
    ...
    World2.set_cell_score(s, a, Q[s][a])
...
def run():
    ...
    for episode_num in range(40):
        steps = 0
        score = 0
        while not World2.has_restarted():
            # Do the action
    ...
    with open('data/R.csv', 'w', newline='') as csvfile:
