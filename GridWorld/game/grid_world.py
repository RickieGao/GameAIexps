...
reward = 0
walls = []
specials = [(1, 0, "green", 100), (2, 6, "red", -600)]
cell_scores = {}
...
def try_move(dx, dy):
    global player, x, y, score, walk_reward, me, restart, reward
    if restart:
        restart_game()
    new_x = player[0] + dx
    new_y = player[1] + dy
...
def do_action(action):
    global master, reward, restart
    if (action == [1, 0, 0, 0]).all:
        try_move(0, -1)
    if (action == [0, 1, 0, 0]).all:
        try_move(0, 1)
    if (action == [0, 0, 1, 0]).all:
        try_move(-1, 0)
    if (action == [0, 0, 0, 1]).all:
        try_move(1, 0)
    else:
        print("error action!")
    return master, reward, restart
