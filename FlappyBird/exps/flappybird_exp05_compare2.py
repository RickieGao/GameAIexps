...
def trainNetwork(s, readout, W_fc1, W_fc2, sess):
	img_names = gb.glob("image_data//reve_input_image//*.npy")
	img_names.sort()
	...
	for t in range(len(img_names)):
		...
		if action_map[tuple(a_t.tolist())] == img_names[t][-5]:
			diff_array.append(img_names[t])
			diff_count += 1
		if action_map[tuple(a_t.tolist())] == img_names[t][-6]:
			rule_action_array.append(img_names[t])
			rule_action_count += 1
		...
		print("TIMESTEP", t,
			...
			"/ Q_MAX %e" % np.max(readout_t), "/ diff_count", diff_count, "/ rule_action", rule_action_count)

	diff_file = open(r'diff_reve_dqn.txt', 'w')
	for word in diff_array:
		diff_file.write(str(word))
		diff_file.write('\n')
	diff_file.close()

	rule_file = open(r'rule_reve_dqn.txt', 'w')
	for word in rule_action_array:
		rule_file.write(str(word))
		rule_file.write('\n')
	diff_file.close()
