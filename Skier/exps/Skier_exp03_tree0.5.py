...
# import copy
# import gym
import skierGame as game
import ruleAction_avoidTree


sys.path.append("game/")
GAME = 'skier-rule'  # the name of the game being played for log files
...
def trainNetwork(s, readout, W_fc1, W_fc2, sess):
    ...
    writer = tf.summary.FileWriter(r"Exp-03-avoidTreeRule-0.5/ExpGraph03/", sess.graph)
    ...
    checkpoint = tf.train.get_checkpoint_state("Exp-03-avoidTreeRule-0.5/Exp03_saved_networks")
    #
    ...
    while True:
        ...
        if t % FRAME_PER_ACTION == 0:
            # choose action based on rule with a probability of omega
            if random.random() <= omega:
                action = ruleAction_avoidTree.rule_action(game_frame)
                if action == "left":
                    a_t[1] = 1
                elif action == "right":
                    a_t[2] = 1
        ...
        if t % 10000 == 0:
            saver.save(sess, r'Exp-02-getFlagRule-0.5/Exp03_saved_networks/' + GAME + '-dqn', global_step=t)
