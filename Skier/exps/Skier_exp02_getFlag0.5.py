...
def trainNetwork(s, readout, W_fc1, W_fc2, sess):
    ...
    writer = tf.summary.FileWriter(r"Exp-02-getFlagRule-0.5/ExpGraph02/", sess.graph)
    ...
    checkpoint = tf.train.get_checkpoint_state("Exp-02-getFlagRule-0.5/Exp02_saved_networks")
    #
    ...
    while True:
        ...
        if t % FRAME_PER_ACTION == 0:
            ...
            if random.random() <= omega:
                action = ruleAction_GetFlag.rule_action(game_frame)
                if action == "left":
                    a_t[1] = 1
                elif action == "right":
                    a_t[2] = 1
        ...
        if t % 10000 == 0:
            saver.save(sess, r'Exp-02-getFlagRule-0.5/saved_networks/' + GAME + '-dqn', global_step=t)
