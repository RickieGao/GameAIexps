...
def trainNetwork(s, readout, W_fc1, W_fc2, sess):
    ...
    writer = tf.summary.FileWriter("tensorboardgraph/", sess.graph)

    # record the reward
    with tf.name_scope('reward per life'):
        reward = tf.Variable(0.0, name='reward')
        reward_sum = tf.summary.scalar('reward per life', reward)

    # record the reward every 1000 time steps
    with tf.name_scope('reward per 10000 steps'):
        reward_step = tf.Variable(0.0, name='reward_step')
        reward_sum_step = tf.summary.scalar('reward per 10000 steps', reward_step)
